...
void GameTask::GameUpdate()
{
	...
	if (_startFlag)
	{
		...
		AddCountRaceTime(_playerRapCnt);
		...
		if (i == p[p.size() - 1])
		{
			if (_startFlag)
			{
				...
				if (i->HitGoalCollision(goalCheck._goalStart, goalCheck._goalEnd))
				{
					if (!_rapRagFlag)
					{
						_playerRanking[_rap] = _playerRapCnt;
						if (_rap > 0)
						{
							/*_playerRanking[_rap][0] -= _playerRanking[_rap -1][0];
							_playerRanking[_rap][1] -= _playerRanking[_rap - 1][1];
							_playerRanking[_rap][2] -= _playerRanking[_rap - 1][2];
							_playerRanking[_rap][3] -= _playerRanking[_rap - 1][3];
							_playerRanking[_rap][4] -= _playerRanking[_rap - 1][4];*/

						}
						_rap++;
						_playerRapCnt = { 0,0,0,0,0 };
						_rapRagFlag = true;
					}
	...
	{
		DrawFormatString(1, 51 + (20 * i), 0x000000, "%d:%d%d:%d%d", _playerRanking[i][4], _playerRanking[i][3], _playerRanking[i][2], _playerRanking[i][1], _playerRanking[i][0]);
		DrawFormatString(0, 50 + (20 * i), 0xffffff, "%d:%d%d:%d%d", _playerRanking[i][4], _playerRanking[i][3], _playerRanking[i][2], _playerRanking[i][1], _playerRanking[i][0]);
	}

	for (int i = 0; i < _raceRanking.size(); i++)
	{
		DrawFormatString(121, 51 + (20 * i), 0x000000, "%d:%d%d:%d%d", _raceRanking[i][4], _raceRanking[i][3], _raceRanking[i][2], _raceRanking[i][1], _raceRanking[i][0]);
		DrawFormatString(120, 50 + (20 * i), 0xffffff, "%d:%d%d:%d%d", _raceRanking[i][4], _raceRanking[i][3], _raceRanking[i][2], _raceRanking[i][1], _raceRanking[i][0]);
	}
	SetFontSize(16);
