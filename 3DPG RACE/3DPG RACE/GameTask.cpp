...
void GameTask::GameOption()
{
	...
	if (_option->GetOption() == SelectOption::MAX && KeyMng::GetInstance().trgKey[P1_SPACE])
	{
		if (!CheckSoundMem(SOUND_ID("sounds/menu1.mp3")))
		{
			PlaySoundMem(SOUND_ID("sounds/menu1.mp3"), DX_PLAYTYPE_BACK);
...
void GameTask::GameResult()
{
	...
	rankingMax = (_topRaceCnt[0] * 0.01f) + (_topRaceCnt[1] * 0.1f) + (_topRaceCnt[2]) + (_topRaceCnt[3] * 10) + (_topRaceCnt[4] * 100);
